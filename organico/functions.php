<?php
/**
 * Organico functions and definitions
 *
 ...
 */
/**
 * Organico only works in WordPress 4.8 or later.
 */
...
if (class_exists('Elementor\Plugin')) {
    global $post;
    if ( is_page() && Plugin::$instance->db->is_built_with_elementor($post->ID) ) {
        add_filter('body_class', function($classes){
            return array_merge( $classes, array( 'ftc-elementor-editor' ) );
        });
    }
}
...
if (!function_exists('ftc_ajax_search')) {
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'organico'), $results->found_posts);
            ...
            $html = '<div class="eror-search"><span class="error">'.esc_html__('No item found.', 'organico').'</span></div';
            $return = array();
            $return['html'] = $html;
            $return['search_string'] = $search_string;
            wp_die(json_encode($return));
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'organico') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'organico') ?></button>
    </form>
    <?php
}
...
function ftc_setup() {
 ...
 add_theme_support( 'editor-font-sizes', array(
    array(
        'name' => __( 'Small', 'organico' ),
        'size' => 12,
        'slug' => 'small'
    ),
    array(
        'name' => __( 'Normal', 'organico' ),
        'size' => 14,
        'slug' => 'normal'
    ),
    array(
        'name' => __( 'Large', 'organico' ),
        'size' => 36,
        'slug' => 'large'
    ),
    array(
        'name' => __( 'Huge', 'organico' ),
        'size' => 48,
        'slug' => 'huge'
    )
) );
 ...
 add_theme_support( 'editor-color-palette', array(
    array(
        'name' => __( 'strong magenta', 'organico' ),
        'slug' => 'strong-magenta',
        'color' => '#a156b4',
    ),
    array(
        'name' => __( 'light grayish magenta', 'organico' ),
        'slug' => 'light-grayish-magenta',
        'color' => '#d0a5db',
    ),
    array(
        'name' => __( 'very light gray', 'organico' ),
        'slug' => 'very-light-gray',
        'color' => '#eee',
    ),
    array(
        'name' => __( 'very dark gray', 'organico' ),
        'slug' => 'very-dark-gray',
        'color' => '#444',
    ),
) );
     ...
     * If you're building a theme based on Organico, use a find and replace
     * to change 'organico' to the name of your theme in all the template files.
     */
    load_theme_textdomain('organico');
    ...
    /* Translation */
    load_theme_textdomain('organico', get_template_directory() . '/languages');
    ...
    register_nav_menus(array(
        'primary' => __('Primary Navigation', 'organico'),
        'vertical' => __('Vertical Navigation', 'organico'),
    ));
    ...
    $starter_content = array(
        ...
        'attachments' => array(
            'image-espresso' => array(
                'post_title' => _x('Espresso', 'Theme starter content', 'organico'),
                'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
            ),
            'image-sandwich' => array(
                'post_title' => _x('Sandwich', 'Theme starter content', 'organico'),
                'file' => 'assets/images/sandwich.jpg',
            ),
            'image-coffee' => array(
                'post_title' => _x('Coffee', 'Theme starter content', 'organico'),
        ...
        'nav_menus' => array(
            ...
            'social' => array(
                'name' => __('Social Links Menu', 'organico'),
                'items' => array(
                    'link_yelp',
                    'link_facebook',
                    'link_twitter',
...
function ftc_content_width() {
    ...
    /**
     * Filter Organico content width of the theme.
     *
     * @since Organico 1.0
     *
...
function ftc_fonts_url() {
     ...
     */
    $dosis = _x( 'on', 'Roboto font: on or off', 'organico' );
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
        /* translators: %s: Name of current post */ sprintf(__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'organico'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
    ...
    wp_enqueue_script('prettyPhoto', get_template_directory_uri() . '/assets/js/prettyPhoto.js', array(), null, true);
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = __('Expand child menu', 'organico');
        $ftc_l10n['collapse'] = __('Collapse child menu', 'organico');
          ...
          <div class="ftc-cookies-popup">
            <div class="ftc-cookies-inner">
               <div class="cookies-info-text">
                ...
                <p><?php echo esc_html(do_shortcode($smof_data['cookies_text'])); ?></p>
            </div>
            <div class="cookies-buttons">
              <a href="#" class="btn btn-size-small btn-color-primary cookies-accept-btn"><?php esc_html_e( "Yes, I Accept" , 'organico' ); ?></a>
          </div>
      </div>
  </div>
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        $options = '<option value="">' . esc_html__('All categories', 'organico') . '</option>';
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'organico');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'organico')
            , '404' => esc_html__('Error 404', 'organico')
            , 'tagged' => esc_html__('Tagged ', 'organico')
            , 'author' => esc_html__('Articles posted by ', 'organico')
            , 'page' => esc_html__('Page', 'organico')
            , 'portfolio' => esc_html__('Portfolio', 'organico')
        );
        ...
        <?php if( !is_page_template('page-blank.php') ): ?>
            <div class="ftc-mobile-wrapper">
                ...
                <div class= "menu-text"> 
                    ...
                    <?php esc_html_e('Menu', 'organico') ?>
                </div>

                <div class="mobile-menu-wrapper">
                    <?php wp_nav_menu( array( 'menu' => 'main menu', 'menu_id' => 'main-menu', 'menu_class' => 'ftc-smartmenu ftc-simple') ); ?>
                </div>
                <?php
                global $smof_data, $woocommerce;
                if (isset($smof_data['ftc_mobile_menu']) && $smof_data['ftc_mobile_menu']): 
                 ?>
...
function ftc_comments_list($comment,$args,$depth){
...
<div class="comment-box media-body">
   <div class="comment-author meta">
    <span class="author-comment"><strong><?php echo get_comment_author_link() ?></strong></span>
    <span class="date"><?php printf(esc_html__('%1$s', 'organico'), get_comment_date()) ?></span>
    <?php edit_comment_link(esc_html__(' - Edit', 'organico'),'  ','') ?>

</div>
<span class="reply-comment"><?php comment_reply_link(array_merge( $args, array( 'reply_text' => esc_html__(' Reply', 'organico'), 'add_below' => 'comment', 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?></span>
<div class="comment-text">
    <?php if ($comment->comment_approved == '0') : ?>
        <em><?php esc_html_e('Your comment is awaiting moderation.', 'organico') ?></em>
        <br />
    <?php endif; ?>
    <?php comment_text() ?>
</div>
...
function ftc_register_required_plugins() {
    ...
    $plugins = array(
        array(
            ...
            'version' => '1.1.2', // E.g. 1.1.0. If set, the active plugin must be this version or higher.
        ...
        ,array(
            ...
            'required' => true, // If false, the plugin is only 'recommended' instead of required.
            'version' => '1.0.1', // E.g. 1.1.0. If set, the active plugin must be this version or higher.
            'force_activation' => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url' => '', // If set, overrides default API URL and points to an external URL.
        )
        ...
        , array(
            ...
            'required' => false, // If false, the plugin is only 'recommended' instead of required.
            'version' => '6.2.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation' => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url' => '', // If set, overrides default API URL and points to an external URL.
        )
        , array(
                ...
                'source' => 'http://demo.themeftc.com/organico-woo/content/ftc-importer-organico-'.$version .'.zip', // The plugin source.
                'required' => true, // If false, the plugin is only 'recommended' instead of required.
        ...
        , array(
                'name' => 'Woo Variation Swatches', // The plugin name.
