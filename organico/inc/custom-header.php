...
function ftc_custom_header_setup() {
	...
	/**
	 * Filter Organico custom-header support arguments.
	 *
	 * @since Organico 1.0
	 *
	 * @param array $args {
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => __( 'Default Header Image', 'organico' ),
		),
	) );
}
...
function ftc_video_controls( $settings ) {
	$settings['l10n']['play'] = '<span class="screen-reader-text">' . __( 'Play background video', 'organico' ) . '</span>' . ftc_get_svg( array( 'icon' => 'play' ) );
	$settings['l10n']['pause'] = '<span class="screen-reader-text">' . __( 'Pause background video', 'organico' ) . '</span>' . ftc_get_svg( array( 'icon' => 'pause' ) );
	return $settings;
}
