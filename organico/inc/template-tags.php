...
function ftc_time_link() {
	...
	return sprintf(
		...
		__( '<span class="screen-reader-text">Posted on</span> %s', 'organico' ),
				...
				if ( ( $categories_list && ftc_categorized_blog() ) || $tags_list ) {
					echo '<span class="cat-tags-links">';
						...
						if ( $categories_list && ftc_categorized_blog() ) {
							echo '<span class="cat-links">' . ftc_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', 'organico' ) . '</span>' . $categories_list . '</span>';
						}

						if ( $tags_list ) {
							echo '<span class="tags-links">' . ftc_get_svg( array( 'icon' => 'hashtag' ) ) . '<span class="screen-reader-text">' . __( 'Tags', 'organico' ) . '</span>' . $tags_list . '</span>';
						}
...
function ftc_edit_link() {
	...
	$link = edit_post_link(
		sprintf(
			...
			__( 'Edit<span class="screen-reader-text"> "%s"</span>', 'organico' ),
			get_the_title()
		),
...
function ftc_front_page_section( $partial = null, $id = 0 ) {
	...
	if ( get_theme_mod( 'panel_' . $id ) ) {
		...
		echo '<article class="panel-placeholder panel ftc-panel ftc-panel' . $id . '" id="panel' . $id . '"><span class="ftc-panel-title">' . sprintf( __( 'Front Page Section %1$s Placeholder', 'organico' ), $id ) . '</span></article>';
	}
}
