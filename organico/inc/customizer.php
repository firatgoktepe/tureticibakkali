...
function ftc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'organico' ),
		'choices'  => array(
			'light'  => __( 'Light', 'organico' ),
			'dark'   => __( 'Dark', 'organico' ),
			'custom' => __( 'Custom', 'organico' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_section( 'theme_options', array(
		'title'    => __( 'Theme Options', 'organico' ),
	...
	$wp_customize->add_control( 'page_layout', array(
		'label'       => __( 'Page Layout', 'organico' ),
		'section'     => 'theme_options',
		'type'        => 'radio',
		'description' => __( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'organico' ),
		'choices'     => array(
			'one-column' => __( 'One Column', 'organico' ),
			'two-column' => __( 'Two Column', 'organico' ),
		),
		'active_callback' => 'ftc_is_view_with_layout_option',
	) );

	/**
	 * Filter number of front page sections in Organico.
	 *
	 * @since Organico 1.0
	 *
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( __( 'Front Page Section %d Content', 'organico' ), $i ),
			'description'    => ( 1 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'organico' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
