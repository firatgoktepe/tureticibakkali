...
if ( ! function_exists( 'carna_popup_newsletter' ) ) {
    ...
    echo '</div>';
    echo '<span class="dont_show_popup"><input id="ftc_dont_show_again" type="checkbox"><label for="ftc_dont_show_again">' .esc_attr__('Don\'t show popup again', 'organico'). '</label></span>';
    echo '</div>';
}
}
/* * * Tiny account ** */
if (!function_exists('ftc_tiny_account')) {
    ...
    function ftc_tiny_account() {
        ...
        <div class="ftc-account">
            <div class="ftc_login">
                <?php if (!$_user_logged): ?>
                    <a  class="login" href="<?php echo esc_url($login_url); ?>" title="<?php esc_html_e('Login', 'organico'); ?>"><span><?php esc_html_e('Login', 'organico'); ?></span></a>
                    / 
                    <a class="ftc_sign_up" href="<?php echo esc_url($register_url); ?>" title="<?php esc_html_e('Create New Account', 'organico'); ?>"><span><?php esc_html_e('Sign up', 'organico'); ?></span></a>
                    <?php else: ?>
                        <a class="my-account" href="<?php echo esc_url($profile_url); ?>" title="<?php esc_html_e('My Account', 'organico'); ?>"><span><?php esc_html_e('My Account', 'organico'); ?></span></a> /
                        <a class="log-out" href="<?php echo esc_url($logout_url); ?>" title="<?php esc_html_e('Logout', 'organico'); ?>"><span><?php esc_html_e('Logout', 'organico'); ?></span></a>
                    <?php endif; ?>
                </div>
                <?php if (!$_user_logged): ?>
                    <div class="ftc_account_form dropdown-container">
                        <form name="ftc-login-form" class="ftc-login-form" action="<?php echo esc_url(wp_login_url()); ?>" method="post">
                            ...
                            <p class="login-username">
                                <label><?php esc_html_e('Username', 'organico'); ?></label>
                                <input type="text" name="log" class="input" value="" size="20" autocomplete="off">
                            </p>
                            <p class="login-password">
                                <label><?php esc_html_e('Password', 'organico'); ?></label>
                                <input type="password" name="pwd" class="input" value="" size="20">
                            </p>

                            <p class="login-submit">
                                <input type="submit" name="wp-submit" class="button-secondary button" value="<?php esc_html_e('Login', 'organico'); ?>">
                                <input type="hidden" name="redirect_to" value="<?php echo home_url($redirect_to); ?>">
                            </p>

                        </form>

                        <p class="ftc_forgot_pass"><a href="<?php echo esc_url(wp_lostpassword_url()); ?>" title="<?php esc_html_e('Forgot Your Password?', 'organico'); ?>"><?php esc_html_e('Forgot Your Password?', 'organico'); ?></a></p>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <?php if(isset($smof_data['ftc_cart_layout']) && $smof_data['ftc_cart_layout'] == 'off-canvas'): ?>
        <div class="ftc-off-canvas-cart">
            <div class="off-canvas-cart-title">
                <div class="title"><?php esc_html_e('Shopping Cart', 'organico'); ?></div>
                <a href="#" class="close-cart"> <?php esc_html_e('Close', 'organico') ?></a>
            </div>
...
function ftc_cart_total() {
    ...
    <div class="total-cart">
       ...
       <span class="cart-number-header8"><span class="text-header8"><?php esc_html_e('My cart', 'organico'); ?></span><?php echo WC()->cart->get_cart_subtotal(); ?></span>
   </div>
   <?php
   return ob_get_clean();
}
...
function ftc_tiny_cart_filter($fragments) {
    $fragments['.total-cart'] = ftc_cart_total();
    return $fragments;
}
...
function ftc_tini_wishlist() {
    ...
    <a title="<?php esc_html_e('Wishlist', 'organico'); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="tini-wishlist">
      <i class="fa fa-heart"></i>  
      <?php esc_html_e('Wishlist', 'organico'); ?> <span class="count-wish"><?php echo '(' . ($count > 0 ? zeroise($count, 2) : '0') . ')'; ?></span>
  </a>
...
if (!function_exists('ftc_woocommerce_multilingual_currency_switcher')) {
    ...
    function ftc_woocommerce_multilingual_currency_switcher() {
        if (class_exists('woocommerce_wpml') && class_exists('WooCommerce') && class_exists('SitePress')) {
            ...
            ?>
            <div class="ftc-currency">
                <a href="javascript: void(0)" class="ftc-currency-selector"><?php esc_html_e('USD','organico'); ?></a>
                <ul>
                    <li rel="USD"><?php esc_html_e('Dollar (USD)','organico'); ?></li>
                    <li rel="EUR"><?php esc_html_e('Euro(EUR)','organico'); ?> </li>
                </ul>
            </div>
            <?php
        }
    ...
    function ftc_wpml_language_selector() {
        if (class_exists('SitePress')) {
            ...
            <div id="ftc_language" class="ftc_language">
                <ul>
                    <li>
                        <a href="#" class="ftc_lang icl-en"><?php esc_html_e('English','organico'); ?></a>
                        <ul style="visibility: hidden;">
                            <li class="icl-fr"><a rel="alternate" href="#"><span class="icl_lang_sel_native"><?php esc_html_e('Francais','organico'); ?> </span></a></li>
                            <li class="icl-de"><a rel="alternate" href="#"><span class="icl_lang_sel_native"><?php esc_html_e('Espanol','organico'); ?></span></a></li>
                        </ul>
                    </li>
                </ul>
            </div>
