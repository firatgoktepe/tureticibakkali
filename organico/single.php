      ...
      <main id="main" class="site-main col-sm-9"  style="<?php if( ($page_column_class['left_sidebar'] && $post->post_type != 'ftc_footer') || ($page_column_class['right_sidebar']) &&  $post->post_type != 'ftc_footer') { ?>width: 75%;<?php }else{ ?>width:100%;<?php } ?>">
        ...
        while ( have_posts() ) : the_post();
          ...
          the_post_navigation( array(
            'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'organico' ) . '</span><span aria-hidden="true" class="nav-subtitle">' . esc_html__( 'Previous', 'organico' ) . '</span> <span class="nav-title"><span class="nav-title-icon-wrapper">' . ftc_get_svg( array( 'icon' => 'arrow-left' ) ) . '</span>%title</span>',
            'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'organico' ) . '</span><span aria-hidden="true" class="nav-subtitle">' . esc_html__( 'Next', 'organico' ) . '</span> <span class="nav-title">%title<span class="nav-title-icon-wrapper">' . ftc_get_svg( array( 'icon' => 'arrow-right' ) ) . '</span></span>',
          ) );
